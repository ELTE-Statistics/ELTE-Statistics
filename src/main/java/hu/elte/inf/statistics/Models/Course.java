...
/**
 * Represents Course Class Course includes: course's name, information about difficulty and
 * usefulness
 */
public class Course {
    ...
    /** @param name Constructor of Course Class with one parameter */
     ...
     * @param usefulnessDataCount Constructor of Course Class with five parameters
     */
    public Course(
            String name,
            double averageDifficulty,
            int difficultyDataCount,
            double averageUsefulness,
            int usefulnessDataCount) {
        this.name = name;
    ...
    /**
     * @param report Adds CourseReport
     * @return false if report doesn't match Course, true otherwise
     */
    public boolean addReport(CourseReport report) {
        if (!report.getCourseName().equals(this.name)) return false;

        averageDifficulty =
                (averageDifficulty * difficultyDataCount + report.getDifficulty())
                        / (double) (difficultyDataCount + 1);
        difficultyDataCount += 1;

        averageUsefulness =
                (averageUsefulness * usefulnessDataCount + report.getUsefulness())
