...
public class CoursesDAO {
    ...
    public boolean addCourseReport(CourseReport report) {
        ...
        this.setCourseAverageDifficulty(
                cName,
                (this.getCourseAverageDifficulty(cName) * diffCount + report.getDifficulty())
                        / (diffCount + 1));
        this.setCourseAverageUsefulness(
                cName,
                (this.getCourseAverageUsefulness(cName) * usflCount + report.getUsefulness())
                        / (usflCount + 1));
        this.setCourseDifficultyCount(cName, this.getCourseDifficultyCount(cName) + 1);
        this.setCourseUsefulnessCount(cName, this.getCourseUsefulnessCount(cName) + 1);
    ...
    public boolean addCourse(Course course) {
        if (this.contains(course.getName())) return false;
    ...
    /**
     * @param name
     * @return Course if database contains course with given name, null otherwise
     */
    ...
    public boolean removeCourse(String courseName) {
        if (!this.contains(courseName)) return false;
    ...
    /**
     * @param courseName
     * @param averageDifficulty Setter for course average difficulty
     */
